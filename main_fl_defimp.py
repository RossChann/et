...
def elastic_training_updated(
    ...
    def compute_dw(x, y):
        ...
        I = I / tf.reduce_max(tf.abs(I))
        global I_G
        I = tf.multiply(I_G, 1 - client_weight) + tf.multiply(I, client_weight)
        for k, w in enumerate(model.trainable_weights):
            w.assign(w_0[k])
        return dw_0, I
...
def federated_elastic_training_advanced(client_datasets, ds_test, model_type='resnet50', global_epochs=4,
    ...
    #######################
    def compute(global_model,averaged_gradients):
        ...
        for x,y in ds_test:
            with tf.GradientTape() as tape:
                y_pred_1 = global_model(x, training=True)
                loss_1 = loss_fn_cls(y, y_pred_1)
            grad_1 = tape.gradient(loss_1, global_model.trainable_weights)
        I = [tf.reduce_sum((grad_1_k * dw_0_k)) for (grad_1_k, dw_0_k) in zip(grad_1, dw_0)]
        I = tf.convert_to_tensor(I)
        I = I / tf.reduce_max(tf.abs(I))
        return I
    ...
    for global_epoch in range(global_epochs):
        ...
        if global_epoch==0:
            ...
            I_G=compute(global_model,averaged_gradients)
            ...
            for client_id, ds_train in enumerate(client_datasets):
                ...
                gradients = elastic_training_updated(client_model, model_name, ds_train, ds_test, run_name='auto', logdir='auto',
                                 timing_info=timing_info, optim='sgd', lr=1e-4, weight_decay=5e-4, epochs=5, interval=5,
                                 rho=0.533, disable_random_id=True, save_model=False, save_txt=False)  # train
