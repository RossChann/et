...
def federated_elastic_training_advanced(client_datasets, ds_test, model_type='resnet50', global_epochs=4,
    ...
    for global_epoch in range(global_epochs):
        ...
        device2_model = full_training(device2_model, client_datasets[1], ds_test, run_name='auto', logdir='auto',
                                      optim='sgd', lr=1e-4, weight_decay=5e-4, epochs=12, disable_random_id=False,
                                      save_model=False, save_txt=False)
        send_weights(device2_model.get_weights(), server_host, server_port)

        client_weights.append(receive_weights(server_host, server_port))  # 服务器接收客户端模型权重
        compute_weights = aggregate_weights(client_weights)
        update_global_model(global_model, compute_weights)
